...
const ListNotification = ({ data }) => {
  const [actNotificationAlert, setActNotificationAlert] = useGlobal(
    "actNotificationAlert"
  )
  const [actGlobal] = useGlobal("actGlobal")

  useEffect(() => {
    const ActNotiAlert = () => {
      ActService.getNotification().then((resp) => {
        try {
          let datas = resp.data.data.map((val) => val)
          setActNotificationAlert(datas)
        } catch {
          setActNotificationAlert("no data")
        }
      })
    }
  ...
  return (
    <Fragment>
      <Table hover>
        <tbody>
          {actNotificationAlert.length === 0 ? (
            <BoxsList>
              <Empty description={<span>ไม่มีการแจ้งเตือน</span>} />
            </BoxsList>
          ) : (
            actNotificationAlert.map((data, index) => (
              <tr key={index}>
                <th
                  scope="row"
